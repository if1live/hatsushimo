...
namespace Hatsushimo.NetChan
{
    class BandwidthList
    {
        ...
        internal int GetBytes(long ts, int millis)
        {
            ...
            var bottom = new Pair() { ts = pair.Item1, bytes = 0 };
            var top = new Pair() { ts = pair.Item2, bytes = 0 };
        ...
        Tuple<long, long> GetTimestampRange(long ts, int millis)
        {
            var bottom = ts - millis + 1;
            var top = ts + 1;
